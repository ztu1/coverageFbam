	...
	for(my $split_rownum = 0; $split_rownum < scalar(@alt_alleles); $split_rownum++){ 
		...
		for(my $j = 0; $j < scalar(@info_keys); $j++){
			if(exists $info_values{$info_keys[$j]}){
				...
				if($info_keys[$j] eq "AC"  or $info_keys[$j] eq "AF"  or $info_keys[$j] eq "AC_AFR"  or $info_keys[$j] eq "AC_AMR" or $info_keys[$j] eq "AC_Adj" or $info_keys[$j] eq "AC_EAS"  or $info_keys[$j] eq "AC_FIN" or $info_keys[$j] eq "AC_Het"  or $info_keys[$j] eq "AC_Hom" or $info_keys[$j] eq "AC_NFE"  or $info_keys[$j] eq "AC_SAS" or $info_keys[$j] eq "AN_AFR" or $info_keys[$j] eq "AN_AMR"  or $info_keys[$j] eq "AN_Adj" or $info_keys[$j] eq "AN_EAS" or $info_keys[$j] eq "AN_FIN" or $info_keys[$j] eq "AN_SAS" or $info_keys[$j] eq "Hom_AFR" or $info_keys[$j] eq "Hom_AMR" or $info_keys[$j] eq "Hom_EAS"  or $info_keys[$j] eq "Hom_FIN"  or $info_keys[$j] eq "Hom_NFE" or $info_keys[$j] eq "Hom_SAS"  or $info_keys[$j] eq "Het_AFR" or $info_keys[$j] eq "Het_AMR" or $info_keys[$j] eq "Het_EAS"  or $info_keys[$j] eq "Het_FIN"  or $info_keys[$j] eq "Het_NFE" or $info_keys[$j] eq "Het_SAS" or $info_keys[$j] eq "EA_AC" or $info_keys[$j] eq "AA_AC" or $info_keys[$j] eq "TAC" or $info_keys[$j] eq "MAF" ){ 
